...
def generate_2d_graph(n, coef=False, delete=True, show=False):
    ...
    graph = max(connected_component_subgraphs, key=len).copy()
...
class App:
    ...
    def addCustomers(self, ncustomer, proportion):
        initquantity = 0

        for perc, freq in proportion:
            quantity = int(perc * ncustomer)
            for i in range(initquantity, initquantity+quantity):
                c = Customer(i, freq)
                agentList.append(c)
    ...
    def addRestaurants(self, nrestaurants, proportion):
        initquantity = 0

        for perc, cap in proportion:
            quantity = int(perc * nrestaurants)
            for i in range(initquantity, initquantity+quantity):
                c = Restaurant(i, cap)
                agentList.append(c)
    ...
    def addDistributor(self, ndistributor):
...
def main():
    ...
    # 10%: 96 ticks (24 horas)
    proportionCustomer = [(0.7, 32), (0.2, 48), (0.1, 96)]
